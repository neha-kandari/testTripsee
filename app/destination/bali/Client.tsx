...
const BaliPage = ({ initialPackages = [] }: { initialPackages?: any[] }) => {
  ...
  const handlePackageClick = async (packageData: { title: string; id?: string }) => {
    console.log('Package clicked:', packageData);     console.log('Package ID:', packageData.id);     console.log('Package title:', packageData.title);     try {
    ...
    if (route) {
      ...
      if (packageData.id) {
        console.log('No hardcoded route, trying dynamic itinerary for packageId:', packageData.id);         router.push(`/itinerary/bali/dynamic/${packageData.id}`);
      } else {
