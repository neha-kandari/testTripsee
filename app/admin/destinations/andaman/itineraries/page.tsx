...
const AndamanItinerariesPage = () => {
  ...
  const fetchItineraries = async () => {
    try {
      const response = await fetch('/api/admin/itineraries?destination=andaman');
      if (response.ok) {
        const data = await response.json();
        const itineraries = data.itineraries || data.itinerary || data;
        
        // Ensure each itinerary has an 'id' field for React keys
        const processedItineraries = itineraries.map((itinerary: any) => ({
          ...itinerary,
          id: itinerary.id || itinerary._id || `itinerary-${Math.random()}`
        }));
      ...
      console.error('Failed to fetch Andaman itineraries:', error);
      setItineraries([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/destinations/andaman/itineraries/${id}`, {
        method: 'DELETE',
      });
    ...
    return (
      <AdminLayout>
        <div className="flex items-center justify-center h-64">
          <div className="text-gray-500">Loading Andaman itineraries...</div>
        </div>
      </AdminLayout>
    );
  }
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Andaman Itineraries</h1>
            <p className="text-gray-600 mt-2">Manage travel itineraries for Andaman</p>
          </div>
          <Link
            href="/admin/destinations/andaman/itineraries/new"
        ...
        {itineraries.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">Create your first Andaman itinerary to get started</p>
            <Link
              href="/admin/destinations/andaman/itineraries/new"
                ...
                <div className="p-6">
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex space-x-2">
                      <Link
                        href={`/admin/destinations/andaman/itineraries/edit/${itinerary.id}`}
...
export default AndamanItinerariesPage;
