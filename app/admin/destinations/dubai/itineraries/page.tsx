...
const DubaiItinerariesPage = () => {
  ...
  const fetchItineraries = async () => {
    try {
            const response = await fetch('/api/admin/destinations/dubai/itineraries');
      
      if (response.ok) {
        const data = await response.json();
                // Transform itineraries to match expected format
        const transformedItineraries = data.map((it: any) => ({
          id: it._id.toString(),
          title: it.title,
        ...
        console.error('Failed to fetch itineraries from API:', response.status);
        setItineraries([]);
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
            const response = await fetch(`/api/admin/destinations/dubai/itineraries/${id}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        console.error('Failed to delete itinerary:', errorData);
    ...
    return (
      <AdminLayout>
        <div className="flex items-center justify-center h-64">
          <div className="text-gray-500">Loading Dubai itineraries...</div>
        </div>
      </AdminLayout>
    );
  }
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Dubai Itineraries</h1>
            <p className="text-gray-600 mt-2">Manage travel itineraries for Dubai</p>
          </div>
          <Link
            href="/admin/destinations/dubai/itineraries/new"
        ...
        {itineraries.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">Create your first Dubai itinerary to get started</p>
            <Link
              href="/admin/destinations/dubai/itineraries/new"
                ...
                <div className="p-6">
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex space-x-2">
                      <Link
                        href={`/admin/destinations/dubai/itineraries/edit/${itinerary.id}`}
...
export default DubaiItinerariesPage;
