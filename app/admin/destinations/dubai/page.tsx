...
const DubaiAdminPage = () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/packages?destination=dubai&t=${timestamp}`);
        ...
        const data = await response.json();
                ...
                // Filter out packages with invalid IDs and transform MongoDB packages to match the expected format (exactly like Vietnam)
        const validPackages = apiPackages.filter((pkg: any) => {
          const hasValidId = pkg.id && pkg.id !== 'undefined' && pkg.id !== 'null';
          if (!hasValidId) {
            console.warn('DUBAI PACKAGE FILTER: Skipping package with invalid ID:', pkg);
          }
          return hasValidId;
        });
        ...
        console.error('Failed to fetch packages from API:', response.status);
        setPackages([]);
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
            // Use centralized API for deletion (exactly like Bali)
      ...
      if (response.ok) {
        ...
        console.error('Failed to delete package:', errorData);
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Dubai Packages</h1>
            <p className="text-gray-600 mt-2">Manage travel packages for Dubai</p>
          </div>
          <div className="flex space-x-3">
            <Link
              href="/admin/destinations/dubai/itineraries"
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          {packages.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-gray-500 mb-4">No Dubai packages were found in the database.</p>
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                <p className="text-sm text-yellow-800">
                  <strong>Debug Info:</strong> Check the browser console for detailed logs about the API response.
                  If packages exist but aren't showing, there might be an issue with the data format or API endpoint.
                </p>
              </div>
              <Link
                href="/admin/destinations/dubai/packages/new"
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {packageData.features && packageData.features.length > 0 && packageData.features.slice(0, 3).map((feature, index) => (
                          <span key={`${packageData.id}-feature-${index}-${feature}`} className="text-xs text-gray-600 bg-gray-100 px-2 py-1 rounded">
                            {feature}
                          </span>
                        ))}
                        {packageData.features && packageData.features.length > 3 && (
                          <span key={`${packageData.id}-more-features`} className="text-xs text-gray-500">+{packageData.features.length - 3} more</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <Link
                        href={`/admin/destinations/dubai/packages/edit/${packageData.id}`}
