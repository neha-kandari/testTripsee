...
const SingaporeAdminPage = () => {
  ...
  const fetchPackages = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch packages from MongoDB API:', response.status);
        setPackages([]);
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this package? This action cannot be undone.')) {
      try {
        const response = await fetch(`/api/admin/destinations/singapore/packages/${id}`, {
          method: 'DELETE',
        });
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Singapore Packages</h1>
            <p className="text-gray-600 mt-2">Manage travel packages for Singapore</p>
          </div>
          <div className="flex space-x-3">
            ...
            <Link
              href="/admin/destinations/singapore/packages/new"
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          {packages.length === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="text-gray-500 mb-6">Get started by creating your first Singapore package</p>
              <Link
                href="/admin/destinations/singapore/packages/new"
                ...
                <div key={packageData.id} className="p-6 hover:bg-gray-50 transition-colors duration-200">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <Link
                        href={`/admin/destinations/singapore/packages/edit/${packageData.id}`}
...
export default SingaporeAdminPage; 
